...
public sealed class SanctuaryRuntimeWorkbenchServiceTests
{
    ...
    [Fact]
    public void CreateFormationPhaseBrittlenessAndDurability_WitnessCandidatePositionAcrossFieldPressure()
    {
        var reachService = new GovernedReachRealizationService();
        var (_, _, _, _, _, _, localityWitness, sharedBoundaryMemory, continuityLedger, deformationReceipt, mutualWitness) = CreatePressureBundle();
        var (_, _, _, _, _, _, _, operatorSelection, _, _, _) = CreateCrucibleBundle();
        ...
        var formationPhaseVector = reachService.CreateFormationPhaseVectorReceipt(
            ...
            receiptState: "formation-phase-vector-ready",
            timestampUtc: FixedTimestamp);
        var brittlenessWitness = reachService.CreateBrittlenessWitnessReceipt(
            ...
            warmReactivationDisposition,
            refusalReceipt,
            receiptState: "brittleness-witness-ready",
            timestampUtc: FixedTimestamp);
        var durabilityWitness = reachService.CreateDurabilityWitnessReceipt(
            ...
            variationLedger,
            intentConstraintAlignment,
            receiptState: "durability-witness-ready",
            timestampUtc: FixedTimestamp);

        Assert.StartsWith("formation-phase-vector://", formationPhaseVector.ReceiptHandle, StringComparison.Ordinal);
        Assert.Equal("formation-phase-vector-bound", formationPhaseVector.ReasonCode);
        Assert.Equal("formation-phase-vector-ready", formationPhaseVector.ReceiptState);
        Assert.Equal(candidateLedger.DominantDistanceClass, formationPhaseVector.DominantDistanceClass);
        Assert.Equal(candidateLedger.PromotionCeiling, formationPhaseVector.PromotionCeiling);
        Assert.Equal(6, formationPhaseVector.PhaseAxes.Count);
        Assert.Equal(3, formationPhaseVector.StabilityAxes.Count);
        Assert.Equal(3, formationPhaseVector.ThermalRegions.Count);
        Assert.Equal("warm-governed-phase-space", formationPhaseVector.FormationRegion);
        Assert.True(formationPhaseVector.WarmGovernanceDominant);
        Assert.False(formationPhaseVector.CoolingEligible);
        Assert.True(formationPhaseVector.ReheatingSensitive);

        Assert.StartsWith("brittleness-witness://", brittlenessWitness.ReceiptHandle, StringComparison.Ordinal);
        Assert.Equal("brittleness-witness-bound", brittlenessWitness.ReasonCode);
        Assert.Equal("brittleness-witness-ready", brittlenessWitness.ReceiptState);
        Assert.Single(brittlenessWitness.BrittlePatterns);
        Assert.Equal(3, brittlenessWitness.FractureAxes.Count);
        Assert.Equal(3, brittlenessWitness.OverfitWarnings.Count);
        Assert.True(brittlenessWitness.SceneBoundBrittlenessDetected);
        Assert.True(brittlenessWitness.MisalignmentPressureDetected);
        Assert.True(brittlenessWitness.PrematureCoolingDenied);

        Assert.StartsWith("durability-witness://", durabilityWitness.ReceiptHandle, StringComparison.Ordinal);
        Assert.Equal("durability-witness-bound", durabilityWitness.ReasonCode);
        Assert.Equal("durability-witness-ready", durabilityWitness.ReceiptState);
        Assert.Equal(2, durabilityWitness.DurablePatterns.Count);
        Assert.Equal(3, durabilityWitness.InterlockSignals.Count);
        Assert.Equal(3, durabilityWitness.CoolingBarriers.Count);
        Assert.True(durabilityWitness.DurableUnderVariation);
        Assert.True(durabilityWitness.InterlockDensityEmergent);
        Assert.True(durabilityWitness.ColdPromotionStillWithheld);
    }

    [Fact]
    public void CreateWarmClockRipeningAndCoolingPressure_WitnessTemporalLawInsideWarmLane()
    {
        var reachService = new GovernedReachRealizationService();
        var (_, _, _, _, _, _, localityWitness, sharedBoundaryMemory, continuityLedger, deformationReceipt, mutualWitness) = CreatePressureBundle();
        var (_, _, _, _, _, _, _, operatorSelection, _, _, _) = CreateCrucibleBundle();
        ...
        var warmClockDisposition = reachService.CreateWarmClockDispositionReceipt(
            ...
            classificationLedger,
            warmReactivationDisposition,
            receiptState: "warm-clock-disposition-ready",
            timestampUtc: FixedTimestamp);
        var ripeningStalenessLedger = reachService.CreateRipeningStalenessLedgerReceipt(
            ...
            brittlenessWitness,
            durabilityWitness,
            ledgerState: "ripening-staleness-ledger-ready",
            timestampUtc: FixedTimestamp);
        var coolingPressureWitness = reachService.CreateCoolingPressureWitnessReceipt(
            ...
            formationPhaseVector,
            intentConstraintAlignment,
            receiptState: "cooling-pressure-witness-ready",
            timestampUtc: FixedTimestamp);

        Assert.StartsWith("warm-clock-disposition://", warmClockDisposition.ReceiptHandle, StringComparison.Ordinal);
        Assert.Equal("warm-clock-disposition-bound", warmClockDisposition.ReasonCode);
        Assert.Equal("warm-clock-disposition-ready", warmClockDisposition.ReceiptState);
        Assert.Equal(4, warmClockDisposition.WarmClocks.Count);
        Assert.Equal("ripening-active", warmClockDisposition.RipeningDisposition);
        Assert.Equal("staleness-risk-present", warmClockDisposition.StalenessDisposition);
        Assert.Equal(1, warmClockDisposition.UnresolvedUnknownLoad);
        Assert.True(warmClockDisposition.ReentryClockActive);
        Assert.True(warmClockDisposition.DistanceBurdenStillActive);
        Assert.True(warmClockDisposition.FailureSignatureFreshnessRequired);
        Assert.True(warmClockDisposition.WarmRipeningUnderway);
        Assert.True(warmClockDisposition.StalenessRiskPresent);

        Assert.StartsWith("ripening-staleness-ledger://", ripeningStalenessLedger.LedgerHandle, StringComparison.Ordinal);
        Assert.Equal("ripening-staleness-ledger-bound", ripeningStalenessLedger.ReasonCode);
        Assert.Equal("ripening-staleness-ledger-ready", ripeningStalenessLedger.LedgerState);
        Assert.Equal(2, ripeningStalenessLedger.RipeningPatterns.Count);
        Assert.Single(ripeningStalenessLedger.StalePatterns);
        Assert.Equal(2, ripeningStalenessLedger.RipeningWindowCount);
        Assert.Equal(1, ripeningStalenessLedger.StaleWindowCount);
        Assert.Equal(1, ripeningStalenessLedger.RefreshRequiredCount);
        Assert.True(ripeningStalenessLedger.HonestWarmRipeningPreserved);
        Assert.True(ripeningStalenessLedger.AdministrativeSuspensionDenied);
        Assert.True(ripeningStalenessLedger.FreshConstraintContactStillRequired);

        Assert.StartsWith("cooling-pressure-witness://", coolingPressureWitness.ReceiptHandle, StringComparison.Ordinal);
        Assert.Equal("cooling-pressure-witness-bound", coolingPressureWitness.ReasonCode);
        Assert.Equal("cooling-pressure-witness-ready", coolingPressureWitness.ReceiptState);
        Assert.Equal(3, coolingPressureWitness.CoolingForces.Count);
        Assert.Equal(3, coolingPressureWitness.CoolingBarriers.Count);
        Assert.Equal("pressure-emergent-but-withheld", coolingPressureWitness.PressureDisposition);
        Assert.True(coolingPressureWitness.CoolingPressureEmergent);
        Assert.False(coolingPressureWitness.ColdApproachLawful);
    ...
    [Fact]
    public void CreateLatticeApproachWitness_DistinguishesInterlockFromRecurrentSuccess()
    {
        var reachService = new GovernedReachRealizationService();
        var (_, _, _, _, _, _, localityWitness, sharedBoundaryMemory, continuityLedger, deformationReceipt, mutualWitness) = CreatePressureBundle();
        var (_, _, _, _, _, _, _, operatorSelection, _, _, _) = CreateCrucibleBundle();
        ...
        var interlockDensityLedger = reachService.CreateInterlockDensityLedgerReceipt(
            durabilityWitness,
            variationLedger,
            coldAdmissionEligibilityGate,
            timestampUtc: FixedTimestamp);
        var brittleDurableDifferentiationSurface = reachService.CreateBrittleDurableDifferentiationSurfaceReceipt(
            brittlenessWitness,
            durabilityWitness,
            interlockDensityLedger,
            timestampUtc: FixedTimestamp);
        var coreInvariantLatticeWitness = reachService.CreateCoreInvariantLatticeWitnessReceipt(
            interlockDensityLedger,
        ...
        Assert.StartsWith("interlock-density-ledger://", interlockDensityLedger.LedgerHandle, StringComparison.Ordinal);
        Assert.Equal("interlock-density-ledger-bound", interlockDensityLedger.ReasonCode);
        Assert.Equal("interlock-density-ledger-ready", interlockDensityLedger.LedgerState);
        Assert.Equal(7, interlockDensityLedger.InterlockLinks.Count);
        Assert.Equal(7, interlockDensityLedger.IndependentConstraintLinkCount);
        Assert.Equal(2, interlockDensityLedger.ReentrySurvivalCount);
        Assert.Equal(2, interlockDensityLedger.DurableAlignmentCount);
        Assert.Equal("moderate-interlock-density", interlockDensityLedger.DensityDisposition);
        Assert.False(interlockDensityLedger.DenseInterweaveEmergent);
        Assert.True(interlockDensityLedger.LatticeClaimStillWithheld);

        Assert.StartsWith("brittle-durable-differentiation-surface://", brittleDurableDifferentiationSurface.SurfaceHandle, StringComparison.Ordinal);
        Assert.Equal("brittle-durable-differentiation-surface-bound", brittleDurableDifferentiationSurface.ReasonCode);
        Assert.Equal("brittle-durable-differentiation-surface-ready", brittleDurableDifferentiationSurface.SurfaceState);
        Assert.Single(brittleDurableDifferentiationSurface.BrittleFragments);
        Assert.Equal(2, brittleDurableDifferentiationSurface.DurableKernels.Count);
        Assert.Equal(3, brittleDurableDifferentiationSurface.CoexistingRegions.Count);
        Assert.Equal("mixed-structure-under-review", brittleDurableDifferentiationSurface.SurfaceDisposition);
        Assert.True(brittleDurableDifferentiationSurface.BrittleDurableCoexistenceExposed);
        Assert.True(brittleDurableDifferentiationSurface.AverageReadinessDenied);
        Assert.True(brittleDurableDifferentiationSurface.FullTrustStillWithheld);

        Assert.StartsWith("core-invariant-lattice-witness://", coreInvariantLatticeWitness.ReceiptHandle, StringComparison.Ordinal);
        Assert.Equal("core-invariant-lattice-witness-bound", coreInvariantLatticeWitness.ReasonCode);
        Assert.Equal("core-invariant-lattice-witness-ready", coreInvariantLatticeWitness.ReceiptState);
        Assert.Equal(3, coreInvariantLatticeWitness.CandidateCoreInvariants.Count);
        Assert.Equal(3, coreInvariantLatticeWitness.IdentityAdjacencySignals.Count);
        Assert.Equal("pre-lattice-moderate", coreInvariantLatticeWitness.InterlockPosture);
        Assert.True(coreInvariantLatticeWitness.IdentityAdjacentSignificanceEmergent);
        Assert.True(coreInvariantLatticeWitness.CoreLawSanctificationDenied);
        Assert.False(coreInvariantLatticeWitness.LatticeGradeInvarianceWitnessed);
    }
