...
public sealed class GovernedSeedPostActionServiceEnactmentPacketTests
{
    [Fact]
    public void Materialized_Packet_Preserves_Operational_Action_Identity_And_Enactment_Receipt()
    {
        var service = new GovernedSeedPostActionServiceEnactmentPacketMaterializationService();
        var operationalActionPacket = CreateOperationalActionPacket(
        ...
        var effectEmissionAssessment = CreateEffectEmissionAssessment(
            operationalActionPacket.PacketHandle,
            operationalActionPacket.CandidateId,
            true,
            true);
        var enactmentCommitAssessment = CreateServiceEnactmentCommitAssessment(
            operationalActionPacket.PacketHandle,
            operationalActionPacket.CandidateId,
            enactmentCommitReady: false,
            serviceEnactmentCommitted: false,
            explicitScopePreserved: true);
        var enactmentAssessment = CreateServiceEnactmentAssessment(
            operationalActionPacket.PacketHandle,
            operationalActionPacket.CandidateId,
            GovernedSeedServiceEnactmentDisposition.EffectEmissionAuthorized,
            effectEmissionAuthorized: true,
            serviceEnactmentCommitted: false);
        var enactmentReceipt = CreateServiceEnactmentReceipt(
            operationalActionPacket.PacketHandle,
            operationalActionPacket.CandidateId,
            GovernedSeedServiceEnactmentDisposition.EffectEmissionAuthorized,
            effectEmissionAuthorized: true,
        ...
        var packet = service.Materialize(
            operationalActionPacket,
            effectEmissionAssessment,
            enactmentCommitAssessment,
            enactmentAssessment,
            enactmentReceipt);

        Assert.Equal(operationalActionPacket.CandidateId, packet.CandidateId);
        Assert.Equal(operationalActionPacket.PacketHandle, packet.PostExecutionOperationalActionPacket.PacketHandle);
        Assert.Equal(enactmentReceipt.ReceiptHandle, packet.PostActionServiceEnactmentReceipt.ReceiptHandle);
        Assert.Equal(GovernedSeedServiceEnactmentDisposition.EffectEmissionAuthorized, packet.PostActionServiceEnactmentAssessment.Disposition);
        Assert.Equal(packet.PostActionServiceEnactmentAssessment.Disposition, packet.PostActionServiceEnactmentReceipt.Disposition);
        Assert.True(packet.EffectEmissionAssessment.EffectEmissionAuthorized);
        Assert.False(packet.ServiceEnactmentCommitAssessment.ServiceEnactmentCommitted);
    }

    [Fact]
    public void Materialize_Rejects_Mismatched_Candidate_Identity()
    {
        var service = new GovernedSeedPostActionServiceEnactmentPacketMaterializationService();
        var operationalActionPacket = CreateOperationalActionPacket();
        ...
        var error = Assert.Throws<InvalidOperationException>(() => service.Materialize(
            operationalActionPacket,
            CreateEffectEmissionAssessment(operationalActionPacket.PacketHandle, "candidate://other", true, true),
            CreateServiceEnactmentCommitAssessment(operationalActionPacket.PacketHandle, operationalActionPacket.CandidateId, true, true, true),
            CreateServiceEnactmentAssessment(operationalActionPacket.PacketHandle, operationalActionPacket.CandidateId, GovernedSeedServiceEnactmentDisposition.ServiceEnactmentCommitted, true, true),
            CreateServiceEnactmentReceipt(operationalActionPacket.PacketHandle, operationalActionPacket.CandidateId, GovernedSeedServiceEnactmentDisposition.ServiceEnactmentCommitted, true, true)));

        Assert.Contains("consistent candidate identity", error.Message);
    }

    [Fact]
    public void Materialize_Rejects_Mismatched_Operational_Action_Packet_Identity()
    {
        var service = new GovernedSeedPostActionServiceEnactmentPacketMaterializationService();
        var operationalActionPacket = CreateOperationalActionPacket();
        ...
        var error = Assert.Throws<InvalidOperationException>(() => service.Materialize(
            operationalActionPacket,
            CreateEffectEmissionAssessment("post-execution-operational-action-packet://other", operationalActionPacket.CandidateId, true, true),
            CreateServiceEnactmentCommitAssessment(operationalActionPacket.PacketHandle, operationalActionPacket.CandidateId, true, true, true),
            CreateServiceEnactmentAssessment(operationalActionPacket.PacketHandle, operationalActionPacket.CandidateId, GovernedSeedServiceEnactmentDisposition.ServiceEnactmentCommitted, true, true),
            CreateServiceEnactmentReceipt(operationalActionPacket.PacketHandle, operationalActionPacket.CandidateId, GovernedSeedServiceEnactmentDisposition.ServiceEnactmentCommitted, true, true)));

        Assert.Contains("consistent operational-action packet identity", error.Message);
    }
    ...
    private static GovernedSeedEffectEmissionAssessment CreateEffectEmissionAssessment(
        string packetHandle,
        string candidateId,
        ...
        new(
            ...
            Summary: "effect-emission");

    private static GovernedSeedServiceEnactmentCommitAssessment CreateServiceEnactmentCommitAssessment(
        string packetHandle,
        string candidateId,
        ...
        new(
            ...
            Summary: "service-enactment-commit");

    private static GovernedSeedPostActionServiceEnactmentAssessment CreateServiceEnactmentAssessment(
        string packetHandle,
        string candidateId,
        ...
        new(
            ...
            EffectEmissionAuthorized: effectEmissionAuthorized,
            ServiceEnactmentCommitted: serviceEnactmentCommitted,
            Summary: "post-action-service-enactment");

    private static GovernedSeedPostActionServiceEnactmentReceipt CreateServiceEnactmentReceipt(
        string packetHandle,
        string candidateId,
        GovernedSeedServiceEnactmentDisposition disposition,
        bool effectEmissionAuthorized,
        bool serviceEnactmentCommitted) =>
        new(
            ReceiptHandle: "post-action-service-enactment://packet/a",
            PacketHandle: packetHandle,
            CandidateId: candidateId,
            Disposition: disposition,
            EffectEmissionAuthorized: effectEmissionAuthorized,
            ServiceEnactmentCommitted: serviceEnactmentCommitted,
            Summary: "post-action-service-enactment-receipt");
