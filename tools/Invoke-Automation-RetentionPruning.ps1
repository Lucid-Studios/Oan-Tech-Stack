...
function Get-ProtectedDirectorySet {
    ...
    foreach ($candidate in @(
        ...
        [string] (Get-ObjectPropertyValueOrNull -InputObject $CycleState -PropertyName 'lastPostHabitationHorizonLatticeBundle'),
        [string] (Get-ObjectPropertyValueOrNull -InputObject $CycleState -PropertyName 'lastBoundedHorizonResearchBriefBundle'),
        [string] (Get-ObjectPropertyValueOrNull -InputObject $CycleState -PropertyName 'lastNextEraBatchSelectorBundle'),
        [string] (Get-ObjectPropertyValueOrNull -InputObject $CycleState -PropertyName 'lastInquirySessionDisciplineSurfaceBundle'),
        [string] (Get-ObjectPropertyValueOrNull -InputObject $CycleState -PropertyName 'lastBoundaryConditionLedgerBundle'),
        [string] (Get-ObjectPropertyValueOrNull -InputObject $CycleState -PropertyName 'lastCoherenceGainWitnessReceiptBundle'),
        [string] (Get-ObjectPropertyValueOrNull -InputObject $CycleState -PropertyName 'lastOperatorInquirySelectionEnvelopeBundle'),
        [string] (Get-ObjectPropertyValueOrNull -InputObject $CycleState -PropertyName 'lastBondedCrucibleSessionRehearsalBundle'),
        [string] (Get-ObjectPropertyValueOrNull -InputObject $CycleState -PropertyName 'lastSharedBoundaryMemoryLedgerBundle'),
        [string] (Get-ObjectPropertyValueOrNull -InputObject $CycleState -PropertyName 'lastContinuityUnderPressureLedgerBundle'),
        [string] (Get-ObjectPropertyValueOrNull -InputObject $CycleState -PropertyName 'lastExpressiveDeformationReceiptBundle'),
        [string] (Get-ObjectPropertyValueOrNull -InputObject $CycleState -PropertyName 'lastMutualIntelligibilityWitnessBundle'),
...
$roots = @(
    ...
    [ordered]@{
        path = Resolve-PathFromRepo -BasePath $resolvedRepoRoot -CandidatePath ([string] $cyclePolicy.postHabitationHorizonLatticeOutputRoot)
        keep = [int] $retentionPolicy.keepPostHabitationHorizonLatticeBundles
    },
    [ordered]@{
        path = Resolve-PathFromRepo -BasePath $resolvedRepoRoot -CandidatePath ([string] $cyclePolicy.boundedHorizonResearchBriefOutputRoot)
        keep = [int] $retentionPolicy.keepBoundedHorizonResearchBriefBundles
    },
    [ordered]@{
        path = Resolve-PathFromRepo -BasePath $resolvedRepoRoot -CandidatePath ([string] $cyclePolicy.nextEraBatchSelectorOutputRoot)
        keep = [int] $retentionPolicy.keepNextEraBatchSelectorBundles
    },
    [ordered]@{
        path = Resolve-PathFromRepo -BasePath $resolvedRepoRoot -CandidatePath ([string] $cyclePolicy.inquirySessionDisciplineSurfaceOutputRoot)
        keep = [int] $retentionPolicy.keepInquirySessionDisciplineSurfaceBundles
    },
    [ordered]@{
        path = Resolve-PathFromRepo -BasePath $resolvedRepoRoot -CandidatePath ([string] $cyclePolicy.boundaryConditionLedgerOutputRoot)
        keep = [int] $retentionPolicy.keepBoundaryConditionLedgerBundles
    },
    [ordered]@{
        path = Resolve-PathFromRepo -BasePath $resolvedRepoRoot -CandidatePath ([string] $cyclePolicy.coherenceGainWitnessReceiptOutputRoot)
        keep = [int] $retentionPolicy.keepCoherenceGainWitnessReceiptBundles
    },
    [ordered]@{
        path = Resolve-PathFromRepo -BasePath $resolvedRepoRoot -CandidatePath ([string] $cyclePolicy.operatorInquirySelectionEnvelopeOutputRoot)
        keep = [int] $retentionPolicy.keepOperatorInquirySelectionEnvelopeBundles
    },
    [ordered]@{
        path = Resolve-PathFromRepo -BasePath $resolvedRepoRoot -CandidatePath ([string] $cyclePolicy.bondedCrucibleSessionRehearsalOutputRoot)
        keep = [int] $retentionPolicy.keepBondedCrucibleSessionRehearsalBundles
    },
    [ordered]@{
        path = Resolve-PathFromRepo -BasePath $resolvedRepoRoot -CandidatePath ([string] $cyclePolicy.sharedBoundaryMemoryLedgerOutputRoot)
        keep = [int] $retentionPolicy.keepSharedBoundaryMemoryLedgerBundles
    },
    [ordered]@{
        path = Resolve-PathFromRepo -BasePath $resolvedRepoRoot -CandidatePath ([string] $cyclePolicy.continuityUnderPressureLedgerOutputRoot)
        keep = [int] $retentionPolicy.keepContinuityUnderPressureLedgerBundles
    },
    [ordered]@{
        path = Resolve-PathFromRepo -BasePath $resolvedRepoRoot -CandidatePath ([string] $cyclePolicy.expressiveDeformationReceiptOutputRoot)
        keep = [int] $retentionPolicy.keepExpressiveDeformationReceiptBundles
    },
    [ordered]@{
        path = Resolve-PathFromRepo -BasePath $resolvedRepoRoot -CandidatePath ([string] $cyclePolicy.mutualIntelligibilityWitnessOutputRoot)
        keep = [int] $retentionPolicy.keepMutualIntelligibilityWitnessBundles
    },
    [ordered]@{
        path = Resolve-PathFromRepo -BasePath $resolvedRepoRoot -CandidatePath ([string] $cyclePolicy.inquiryPatternContinuityLedgerOutputRoot)
        keep = [int] $retentionPolicy.keepInquiryPatternContinuityLedgerBundles
    },
    [ordered]@{
        path = Resolve-PathFromRepo -BasePath $resolvedRepoRoot -CandidatePath ([string] $cyclePolicy.questioningBoundaryPairLedgerOutputRoot)
        keep = [int] $retentionPolicy.keepQuestioningBoundaryPairLedgerBundles
    },
    [ordered]@{
        path = Resolve-PathFromRepo -BasePath $resolvedRepoRoot -CandidatePath ([string] $cyclePolicy.carryForwardInquirySelectionSurfaceOutputRoot)
    ...
    [ordered]@{
        path = Resolve-PathFromRepo -BasePath $resolvedRepoRoot -CandidatePath ([string] $cyclePolicy.variationTestedReentryLedgerOutputRoot)
        keep = [int] $retentionPolicy.keepVariationTestedReentryLedgerBundles
    },
    [ordered]@{
        path = Resolve-PathFromRepo -BasePath $resolvedRepoRoot -CandidatePath ([string] $cyclePolicy.questioningAdmissionRefusalReceiptOutputRoot)
        keep = [int] $retentionPolicy.keepQuestioningAdmissionRefusalReceiptBundles
    },
    [ordered]@{
        path = Resolve-PathFromRepo -BasePath $resolvedRepoRoot -CandidatePath ([string] $cyclePolicy.promotionSeductionWatchOutputRoot)
    ...
    [ordered]@{
        path = Resolve-PathFromRepo -BasePath $resolvedRepoRoot -CandidatePath ([string] $cyclePolicy.formationPhaseVectorOutputRoot)
        keep = [int] $retentionPolicy.keepFormationPhaseVectorBundles
    },
    [ordered]@{
        path = Resolve-PathFromRepo -BasePath $resolvedRepoRoot -CandidatePath ([string] $cyclePolicy.brittlenessWitnessOutputRoot)
        keep = [int] $retentionPolicy.keepBrittlenessWitnessBundles
    },
    [ordered]@{
        path = Resolve-PathFromRepo -BasePath $resolvedRepoRoot -CandidatePath ([string] $cyclePolicy.durabilityWitnessOutputRoot)
