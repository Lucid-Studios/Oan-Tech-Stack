...
foreach ($bucketLabel in @($requestContract.targetBucketLabels)) {
    ...
    $evidenceLinks = @(
        [string] $federationPolicy.versionedTouchPointMatrixPath,
...
$statusOutput = & $statusScriptPath -RepoRoot $resolvedRepoRoot -FederationPolicyPath $resolvedFederationPolicyPath
